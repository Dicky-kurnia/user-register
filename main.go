...
func main() {
	...
	campaigns, err := campaignsRepository.FindByUserID(1)
...
func authMiddleware(authService auth.Service, userService user.Service) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		token, err := authService.ValudateToken(tokenString)
		if err != nil {
			response := helper.APIResponse("Unauthorization", http.StatusUnauthorized, "error", nil)
			c.AbortWithStatusJSON(http.StatusUnauthorized, response)
			return
...
//langkah langkah midleware

// ambil nilai header autorization : Bearer tokentokentoken
